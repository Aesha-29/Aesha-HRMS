...
function LeaveRequest() {
    ...
    // Mock Login: In absence of full RBAC yet, simulate employee login
    useEffect(() => {
        const fetchEmps = async () => {
            const res = await axios.get("http://localhost:5000/api/employees");
            setEmployees(res.data.filter((e: any) => e.status === "Active"));
            if (res.data.length > 0) {
                setActiveEmployeeId(res.data[0].id.toString());
            }
        };
        fetchEmps();
    }, []);
    ...
    const fetchData = async (empId: string) => {
        setLoading(true);
        try {
            const balRes = await axios.get(`http://localhost:5000/api/leaves/balances/${empId}`);
            setBalances(balRes.data);

            // fetch own history (filtering from all requests for now)
            const reqRes = await axios.get(`http://localhost:5000/api/leaves/requests`);
    ...
    return (
        <div className="leave-container">
            <div className="leave-header">
                ...
                >
                    {employees.map(e => <option key={e.id} value={e.id}>{e.firstName} {e.lastName} (Simulate login)</option>)}
                </select>
            </div>
