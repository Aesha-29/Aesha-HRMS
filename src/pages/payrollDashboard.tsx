...
function PayrollDashboard() {
    ...
    const fetchRuns = async () => {
        try {
            setLoading(true);
            const res = await axios.get("http://localhost:5000/api/payroll/runs");
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await axios.delete(`http://localhost:5000/api/payroll/runs/${id}`);
            fetchRuns();
        } catch (error) {
    ...
    const viewPayslips = async (id: number, month: string) => {
        try {
            const res = await axios.get(`http://localhost:5000/api/payroll/runs/${id}/payslips`);
