...
function BulkUpdate() {
    ...
    const handleDownloadTemplate = async () => {
        try {
            const res = await axios.get(`http://localhost:5000/api/bulk-update/template?updateType=${updateType}&branchId=${branchId}&departmentId=${departmentId}`, {
                responseType: 'blob'
            });
    ...
    const validateWithBackend = async (data: any[]) => {
        try {
            const res = await axios.post("http://localhost:5000/api/bulk-update/validate", {
                updateType,
                data
            });
    ...
    const handleCommit = async () => {
        setLoading(true);
        try {
            const res = await axios.post("http://localhost:5000/api/bulk-update/apply", {
                updateType,
