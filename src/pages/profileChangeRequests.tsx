...
function ProfileChangeRequests() {
    ...
    const fetchRequests = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`http://localhost:5000/api/profile-changes?status=${filterStatus === 'All' ? '' : filterStatus}`);
    ...
    const handleApprove = async () => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/profile-changes/${selectedRequest.id}/approve`, {
    ...
    const handleReject = async () => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/profile-changes/${selectedRequest.id}/reject`, {
                reviewedBy: 1, // TODO: Get logged in admin ID
                rejectionReason
            });
        ...
        <div className="profile-change-container">
            <div style={{ marginBottom: '32px' }}>
                <h2 style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: 0, marginBottom: '8px' }}>
                    <UserCog size={32} color="#3b82f6" fill="#eff6ff" /> Profile Change Requests
                </h2>
