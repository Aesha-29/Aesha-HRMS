...
function LevelMaster() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (editingId) {
                await axios.put(`http://localhost:5000/api/levels/${editingId}`, payload);
            } else {
                await axios.post("http://localhost:5000/api/levels", payload);
            }
        ...
        <div className="level-master-container">
            <div>
                <h2 style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: 0, marginBottom: '8px' }}>
                    <Layers size={32} color="#3b82f6" fill="#eff6ff" /> Level Master
                </h2>
